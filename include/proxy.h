...
class Proxy {
public:
    Proxy(int port) : port(port), server_fd(-1), running(false), BLACK_LIST(initFilterList("asset/blocked_domains.txt", "asset/blocked_ip.txt")) {}

    void run() {
        if (running && server_fd != INVALID_SOCKET) {
            acceptConnections();
        } else {
            start();
        }
